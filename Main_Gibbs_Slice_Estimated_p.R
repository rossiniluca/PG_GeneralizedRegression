# Main file for the simulation experiments for the Polya-Gamma sampler of a generalized logistic regression
# Written by Dalla Valle, L., Leisen, F., Rossini, L. and Zhu, W. (2019)
rm(list = ls())
...
for(kk in 1:20){
  ...
  # Fit the model
  n.samples <- 3000 #number of iteration of Gibbs Sampler
  burn      <- 1000 #burn-in iteration
  n.slice   <- 10   #number of iteration within Gibbs
  prior.mn  <- 0     #prior mean for beta
  prior.sd  <- 5     #prior std deviation for beta 
