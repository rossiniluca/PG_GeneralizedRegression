...
// [[Rcpp::export]]
double rpg_gamma(double b, double c){
  
  double x = 0;
  for(int i = 1; i < 200; i++){
    double gamma_var = R::rgamma(b, 1);
    x += (gamma_var) / ((i - .5)*(i - .5) + c*c / (4*MATH_PI*MATH_PI)); 
  }
  
  return( (1 / (2*MATH_PI*MATH_PI)) * x);
}

// [[Rcpp::export]]
double rpg(double b, double z){
